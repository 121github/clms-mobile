...
class Leads extends CI_Controller {
  ...
  private $_searchDropdownData = array(
      'postrange' => array(1, 5, 10, 25, 50, 100),
      'call_status' => array(
          'Quote Given',
          'Existing Customer',
          'Duplicate Record',
  ...
  public function acturis_pending() {
    $data = array(
        'pageId' => 'acturis-pending',
        'title' => 'Acturis Pending',
        'leads' => $this->Leads_model->get_leads_pending_acturis()
    );
    $this->template->load('default', 'leads/acturis_pending', $data);
  }
  ...
  public function view() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      if (!$_SESSION['view_leads_filter']) {
        $_SESSION['view_leads_filter'] = array("prospector" => $_SESSION['login'], "call_status" => null, "order_by" => null);
      }
      $_SESSION['view_leads_qry'] = $this->Leads_model->build_view_leads_qry($_SESSION['view_leads_filter']);
    }
    ...
    $total_pages = ceil($_SESSION['view_leads_qry']['total']/50);
    $_SESSION['view_leads_qry']['limitStart'] = ($page_num-1)*50;
    
    for($x=0;$x<=$total_pages-1;$x++){ 
      $page_desc = (($x*50)+1)." to ".(($x*50)+50);
      $page_array[$x] = $page_desc; 
    }
   ...
   if (!empty($page_num)&&!empty($leads)) {
      $title = "Showing records ". $page_array[$page_num-1] ." of " .$qryData['total']; 
    }
  ...
  public function load_advanced_search_data() {
    if ($this->input->is_ajax_request()) {
      ...
      $options['prospector'] = $this->Leads_model->get_prospectors();
  ...
  public function search_results() {
    ...
    $total_pages = ceil($_SESSION['search_leads_qry']['total']/50);
    $_SESSION['search_leads_qry']['limitStart'] = ($page_num-1)*50;
    ...
    foreach($leads as $k => $lead){
      if(isset($lead['nextcontact'])){
        if(strtotime($lead['nextcontact'])<strtotime('now')){
          $leads[$k]['nextcall_days'] = time_elapsed_string(strtotime($lead['nextcontact']));
        } else {
          $leads[$k]['nextcall_days'] = time_togo_string(strtotime($lead['nextcontact']));
        }
      }
      if(isset($lead['date_updated'])){
          $leads[$k]['lastcall_days'] = time_elapsed_string(strtotime($lead['date_updated']));
                            }
    }
  ...
  public function paginate_leads_results() {
    ...
    foreach($leads as $k => $lead){
      if ($_SESSION['login']==$lead['prospector']){
      $leads[$k]['class'] = 'lead-is-mine'; }
      else if($lead['prospector']<>'1'&&!empty($lead['prospector'])){
        $leads[$k]['class'] = 'lead-is-owned';
      } else {
        $leads[$k]['class'] = '';
      }
      ...
      $title = "Showing records ". (($page*50)+1) ." to ". (($page*50)+50); 
if ($_SESSION['page'] == "search") {
         $_SESSION['search_leads_qry'] =$data;
      } else if ($_SESSION['page'] == "view") {
        $_SESSION['view_leads_qry'] =$data;
      }
  ...
  public function detail() {
    ...
    $coname = $this->Leads_model->get_coname($urn);
    $data = array(
        'pageId' => 'lead-detail-' . uniqid(),
        'pageClass' => 'lead-detail',
        'coname' => $coname,
        'title' => $coname . " details",
        'urn' => $urn,
  ...
  public function load_company_details_view() {
    if ($this->input->is_ajax_request()) {
      ...
      $companyDetails = array(
          ...
          'turnover_validated' => array('label' => 'Turnover Validated', 'type' => 'checkbox'),
          'employees' => array('label' => 'Employees', 'options' => $this->_searchDropdownData['employees']),
          'employees_validated' => array('label' => 'Employees Validated', 'type' => 'checkbox'),
          ...
          'consent_to_contact' => array('label' => 'Consent to Contact', 'type' => 'checkbox')
      );
  ...
  public function update_company_details() {
    if ($this->input->is_ajax_request()) {
      ...
      $formData['turnover_validated'] = empty($formData['turnover_validated']) ? 0 : 1;
      $formData['employees_validated'] = empty($formData['employees_validated']) ? 0 : 1;
      $formData['consent_to_contact'] = empty($formData['consent_to_contact']) ? 0 : 1;

      //if they dont have the acturis reference we set it as pending  
      if (!empty($formData['acturis_later'])) {
      ...
      if (!$this->Leads_model->update_company_details($formData)) {
        $this->Leads_model->clean_db($logging);
        echo json_encode(array('success' => false, 'message' => 'Update Failed. Please contact support'));
        exit;
      }

      echo json_encode(array('success' => true, 'message' => 'Update successful.', 'fields' => $formData,'reset'=>$reset));
      exit;
    }
  }
//added a function for validation rules which can be used to check that a value was set in a select
  public function noSelection($str) {
    if ($str == 'no_selection_made') {
      $this->form_validation->set_message('noSelection', 'The %s field must be set');
      return FALSE;
    } else {
      return TRUE;
    }
...
//added this rule to stop policies being added with no selection


      $this->load->library('form_validation');
      $this->form_validation->set_error_delimiters('', '');

      $this->form_validation->set_rules('policy', 'Policy', 'trim|required|callback_noSelection');
      ...
      //if no policy type is selected we set the type as unknown
      $type = $this->input->post('policy')==="no_selection_made"?"Unknown":$this->input->post('policy');
      //put the policy details into an array ready for theinsert
      $data = array(
          'urn' => $this->input->post('urn'),
      ...
      if ($data['id'] == 0) {
        ...
        if (!$insert_id) {
          echo json_encode(array('success' => false, 'message' => 'Failed to add policy. Possible duplicate?'));
          exit;
        }
        ...
        if (!$this->Leads_model->update_policy($data)) {
          $this->Leads_model->clean_db($logging);
          echo json_encode(array('success' => false, 'message' => 'Failed to update policy. Possible duplicate?'));
          exit;
        }
        if ($logging) {
  ...
  public function update_attendees() {
    if ($this->input->is_ajax_request()) {
      ...
      $id = $this->input->post('id');
      $this->Appointment_model->delete_attendees(null, $id);

      foreach ($attendees as $attendee) {
        $this->Appointment_model->update_attendees($attendee, $id);
      }
    }
  }
    ...
    if ($this->input->is_ajax_request()) {
      $urn = $this->input->post('urn');
      $this->Leads_model->reset_record($urn);
    }
  }
